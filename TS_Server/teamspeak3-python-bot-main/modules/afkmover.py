...
bot: Bot.Ts3Bot
autoStart = False
channel_name = "[cspacer01]❤❤❤❤入口 Entrance [💻选频大厅]❤❤❤❤"  # 默认频道名称
...
class AfkMover(Thread):
    ...
    def __init__(self, stop_event, ts3conn):
        ...
        if self.afk_channel is None:
            AfkMover.logger.error("AFK channel not found! AfkMover will not work.")
            self.stopped.set()  # 停止插件运行
            return
        self.client_channels = {}  # 初始化 client_channels
        self.afk_list = None
    ...
    def get_afk_channel(self, name="AFK"):
        ...
        try:
            # 查找频道
            channels = self.ts3conn.channelfind(name)
            if not channels:
                AfkMover.logger.error(f"Channel '{name}' not found!")
                return None
            # 返回第一个匹配频道的 ID
            channel_id = channels[0].get("cid", '-1')
            AfkMover.logger.info(f"Found AFK channel: {name} (ID: {channel_id})")
            return channel_id
        except TS3Exception as e:
            AfkMover.logger.exception(f"Error getting AFK channel: {e}")
            return None
    ...
    def auto_move_all(self):
        ...
        while not self.stopped.wait(2.0):
            if self.afk_channel is None:
                AfkMover.logger.error("AFK channel is not set! AfkMover will not work.")
                break
...
def start_afkmover(self=None, sender=None, msg=None):
    """
...
def stop_afkmover(self=None, sender=None, msg=None):
    """
...
def get_afk_list(self=None, sender=None, msg=None):
    """
...
def client_left(event_data, ts3bot):
    """
...
def setup(ts3bot, **kwargs):
    """
    Setup function for AfkMover plugin.
    """
    global bot, channel_name
    bot = ts3bot
    channel_name = kwargs.get("afk_channel", "[cspacer01]❤❤❤❤入口 Entrance [💻选频大厅]❤❤❤❤")
    if autoStart:
        start_afkmover()
